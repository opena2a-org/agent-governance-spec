...
Anthropic's model specification defines "hardcoded behaviors" as actions the model should always or never do, regardless of operator or user instructions. This maps directly to the AGS Hardcoded Behaviors domain:
...
- SOUL-HB-002 addresses data exfiltration, which Anthropic treats as a safety concern within the broader category of not helping with harmful activities. AGS elevates it to a standalone hardcoded control because tool-using agents have direct access to network endpoints
...
OpenAI's usage policies define similar non-negotiable constraints. AGS Hardcoded Behaviors provides the governance documentation format that makes these constraints explicit and testable in any agent's SOUL.md file.
