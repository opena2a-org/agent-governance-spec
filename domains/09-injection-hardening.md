...
Anthropic's model specification addresses prompt injection through the principal hierarchy: user instructions cannot override developer instructions, and the model should be robust to attempts to manipulate the instruction hierarchy. AGS Injection Hardening provides the declared format for these defenses:
...
OWASP's LLM Top 10 lists prompt injection (LLM01) as the most critical vulnerability. AGS Injection Hardening controls provide a governance-level defense layer that complements model-level and runtime-level defenses.
